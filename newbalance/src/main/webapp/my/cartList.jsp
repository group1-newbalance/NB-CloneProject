...
<link rel="icon" type="image/x-icon"
   ...
   <link rel="stylesheet" href="/newbalance/common/header.css">
	...
	<link rel="stylesheet" href="/newbalance/css/my/cartList.css">
</head>
<body>
...
$(document).ready(function(){
	...
	$("#optionChange").click(function(){
		...
		$.ajax({
			async: false,
			url: "/newbalance/my/updateCartOption.ajx",
			type: "POST",
			data: {
				"pdCode" : $("#pdCode").val()
				,"pdColor" : $("#pdColor").val()
				, "pdSizeCode" : $("#pdSizeCode").val()
				, "cartNum" : $("#cartNum").val()
			},
			dataType: 'json',
	...
	function qtyChange(obj) {
		if(confirm("수량을 변경 하시겠습니까?")) {
			...
			$.ajax({
				async: false,
				url: "/newbalance/my/updateCartQty.ajx",
				type: "POST",
				data: {
					"qty" : qty
					,"cartNum" : cartNum
...
function fOptionChange(pdName, cartNum, pdCode, color, size, prodImg){
	$.ajax({
        url : '/newbalance/my/getCartOption.ajx',
        type : 'POST',   
        async : false,
        dataType : 'json',
        cache : false,
...
$(function() {	
	...
	$("#deleteCartBtn").click(function() {
        ...
        $.ajax({
           url : '/newbalance/my/deleteCartProduct.ajx',
           type : 'POST',   
           async : false,
           dataType : 'json',
           cache : false,
